...
// Where to fetch unified.json from. Three modes, in priority order:
//
// 1. VITE_DATA_URL (set in Cloudflare Pages env vars OR a local .env
//    file) — fetched at runtime. Overrides everything below.
//
// 2. In a production build, default to the canonical GitHub raw URL
//    for the `data` branch's unified.json. Edit DEFAULT_PROD_URL if you
//    fork the repo. Cloudflare's edge cache + GitHub's 5-min raw cache
//    work together to keep this fast and fresh.
//
// 3. In a dev build (npm run dev), default to the local /data/unified.json
//    that scripts/copy-data.mjs writes during prebuild — falls back to
//    the bundled example file if no real data is around.
//
// Because (1)/(2) cross origins, browsers do a CORS preflight on the
...
const DATA_URL =
  ...
  (import.meta.env.PROD
    ? DEFAULT_PROD_URL
    : `${import.meta.env.BASE_URL || '/'}data/unified.json`)
